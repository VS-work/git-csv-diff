...
gitCsvDiff.prototype.process = function (fileName, result, fileModifier, callback) {
  return callback(false, getDiffByFile(fileName, result, fileModifier));
};

gitCsvDiff.prototype.translations = function (result) {
  return generateTranslations(result);
};

// internal

function getDiffByFile(fileName, dataDiff, fileModifier) {
